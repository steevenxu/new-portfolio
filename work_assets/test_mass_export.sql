...
IF (
    SELECT count(*)
    FROM sys.TABLES
    WHERE NAME = 'CustomExportTable'
    ) > 0
    DROP TABLE CustomExportTable

    BEGIN
        CREATE TABLE [CustomExportTable] (
...
IF (
        SELECT OBJECT_ID('CustomExportIRImages')
        ) IS NOT NULL
BEGIN
    DROP PROCEDURE CustomExportIRImages
END
GO

CREATE PROCEDURE CustomExportIRImages
AS
BEGIN
    TRUNCATE TABLE glinvregexportdetail

    INSERT INTO glinvregexportdetail (
        hrecord
        ,ipage
        ,hbatch
        ,hfile
    ...
    WHERE 1 = 1
        ...
        AND g.hmy NOT IN (
            SELECT hrecord
            FROM CustomExportTable
            )

    INSERT INTO CustomExportTable (
        hrecord
        ,dtstamp
        )
    SELECT DISTINCT hrecord
        ,GETDATE()
    FROM GLInvRegExportDetail
END
...
BEGIN 
    ...
    DECLARE @hVolume NUMERIC
    DECLARE @hStepValue INT
	DECLARE @hPropLen NUMERIC
	DECLARE @tempvar VARCHAR
	
	
	SELECT @hPropLen = count(*) FROM listprop l JOIN property p ON p.hmy = l.hproplist WHERE p.scode='all'
	
    set @hStepValue = 1  --export starts on step 1, sp is 0
    ...
    SELECT @hVolume = 
             count(sys.ipage)
                ...
                where
                    p.hmy in (select l.hproperty from listprop l 
                                join property p on p.hmy=l.hproplist 
                                where p.scode = 'all') --IMPORTANT VARIABLE NAME--
                    and gl.istatus = 4
...
SELECT @hTaskStep = (
    SELECT max(hmy)  --changed from min() to max() as the taskstep will grow
    FROM tsstep
    WHERE htstask = @hTask
)
...
/* 	SELECT @tempvar =   
	...
	SELECT @hTaskStep = (
		SELECT max(hmy)  --changing the select to now match the last entry in the task step
		FROM tsstep
		WHERE htstask = @hTask
		)
    ...
    VALUES (
        ...
        ,'&Property='+(SELECT  
					  ...
					  where
						p.scode = 'all'
					  order by pr.scode offset (@hStepValue-1) row
					  FETCH NEXT 1 ROW ONLY)+'&InvDateFrom=01/01/2018&ZipFiles=1&OverrideLimiter=1&ExportPath=\\asp1\hfs\hUsersdefpaths\Nicolosi and Fitch\Live\InvoiceRegisterExport\'+(SELECT  
																																														pr.scode from
																																														  property p
																																														  join listprop l on l.hproplist = p.hmy
																																														  join property pr on pr.hmy = l.hproperty
